...
**v2.0.3 起：粘贴分享链接直接下载**。在微信里「分享 → 复制链接」拿到链接后：

```bash
python run.py --channels-link "https://weixin.qq.com/sph/xxxxxxxx"
```

工具识别链接 → 启动嗅探 → 尝试唤起链接 → 引导在微信中打开；视频号预览页在微信内置浏览器里加载后自动捕获并下载该视频（完成后自动结束会话）。网页控制台「视频号」页也有「分享链接下载」输入框（`POST /api/v1/channels/link`）。分享文案里混排文字时可直接粘贴整段文案，工具会自动抽取其中的链接。
...
**真机实测结论（2026-09-23，Windows 微信 4.1.15）**：微信内视频号页面（`/web/pages/*`）的数据经**原生桥接**下发，不走 HTTP——A/B/C 均拿不到 feed（探针与 `candidate_paths` 可证）；而**分享链接预览页**（`/finder-preview/pages/sph`）在微信内置浏览器里经页面自身 API 取数，注入覆盖后即可稳定捕获（`--channels-link` 链路）。因此推荐用法：**单个目标视频用分享链接；持续刷视频捕获用 `--channels` + 页面按钮**。

**链路诊断（替代「暂无嗅探结果」）**：CLI 会话与网页控制台「视频号」页实时展示九级状态链——代理连接 → 目标域命中 → HTML 拦截 → 脚本注入 → 前端心跳 → 页面按钮 → Feed 获取 → 下载成功，第一个 ✗ 即断点，并附针对性处置建议与各策略供数占比；诊断负载还包含最近命中的响应路径清单与页面运行时探针（hook 安装情况 / 当前 video 直链），真机排障时直接看出数据从哪来。
